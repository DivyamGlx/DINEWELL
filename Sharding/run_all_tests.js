...
async function runTest(test) {
    ...
    return new Promise((resolve) => {
        ...
        child.on('exit', (code) => {
            // Basic check for PASS/FAIL based on console output. A more robust check would parse stdout.
            const passedCount = (stdout.match(/PASS/g) || []).length;
            const failedCount = (stdout.match(/FAIL/g) || []).length;
            const totalCount = passedCount + failedCount;

            if (code === 0 && failedCount === 0 && totalCount >= test.expectedPass) {
                resolve({ name: test.name, status: 'PASS', passed: passedCount, total: totalCount });
            } else {
                resolve({ name: test.name, status: 'FAIL', reason: `Exit code: ${code}, Failed tests: ${failedCount}`, passed: passedCount, total: totalCount });
            }
        });
    });
}
