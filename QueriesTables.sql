-- =============================================================================
-- MESS MANAGEMENT SYSTEM — DATABASE SCHEMA & SEED DATA
-- =============================================================================
...
-- Four messes: Jaiswal (J), Mohani (M), Arbacus (A), Bhopal (B)
CREATE TABLE Messes (
...
CREATE TABLE Student_Mess_Allotment (
    ...
    -- A student may have only one active allotment at a time
...
CREATE TABLE QR_Codes (
    ...
    UNIQUE KEY unique_student_qr (student_id),
...
-- Records guest meal purchases (paid via UPI)
...
-- Auto-generates a formatted bill number after each transaction is inserted
DELIMITER $$
...
-- Monthly student bills; due_amount is computed automatically
CREATE TABLE Bills (
...
CREATE TABLE Admin_Users (
    ...
    mess_id       INT          NULL,   -- NULL for superadmin
    created_date  DATETIME     DEFAULT CURRENT_TIMESTAMP,
...
-- -----------------------------------------------------------------------------
-- 2.1  Messes  (name is a single-letter code stored in the DB)
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- 2.3  Students  (15 students across years 2 and 3)
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- 2.4  Mess Allotments  (all active from 2026-01-01)
-- -----------------------------------------------------------------------------
--      mess_id  1=Jaiswal  2=Mohani  3=Arbacus  4=Bhopal
...
-- -----------------------------------------------------------------------------
-- 2.5  QR Codes  (one per student, valid through 2026)
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- 2.6  Attendance  (sample scan for each student)
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- 2.8  Transactions  (guest meal payments)
-- -----------------------------------------------------------------------------
